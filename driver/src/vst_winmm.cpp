...
#include "VSTDirectMusicSynth.h" // Just for LOG_MSG
...
STDAPI_(LONG) DriverProc(DWORD dwDriverID, HDRVR hdrvr, WORD wMessage, DWORD dwParam1, DWORD dwParam2) {
	...
	case DRV_PNPINSTALL:
		...
		RegisterSynth(CLSID_VSTMIDIDirectMusicSynth, _T("VST MIDI Synth"));
		return DRV_OK;
	...
	case DRV_REMOVE:
		...
		UnregisterSynth(CLSID_VSTMIDIDirectMusicSynth);
		return DRV_OK;
	}
...
HRESULT modGetCaps(PVOID capsPtr, DWORD capsSize) {
	...
	case (sizeof(MIDIOUTCAPS2A)):
		...
		myCaps2A->ProductGuid = CLSID_VSTMIDIDirectMusicSynth;
		myCaps2A->NameGuid = CLSID_VSTMIDIDirectMusicSynth;
		return MMSYSERR_NOERROR;
	...
	case (sizeof(MIDIOUTCAPS2W)):
		...
		myCaps2W->ProductGuid = CLSID_VSTMIDIDirectMusicSynth;
		myCaps2W->NameGuid = CLSID_VSTMIDIDirectMusicSynth;
		return MMSYSERR_NOERROR;
...
bool SetupDirectMusicInterface() {
	...
	if(ntStatus == S_OK) {
		...
		if(myDM != NULL) {
			myDMPort = NULL;
			ntStatus = myDM->CreatePort(CLSID_VSTMIDIDirectMusicSynth, &dummyParms, &myDMPort, NULL);
			if (myDMPort != NULL) {
				myDMBuffer = NULL;
